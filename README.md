...
6. **Panggil Berdasarkan Nama** — pilih sapaan (**- Langsung kepada -** / **Bapak** / **Ibu**), ketik nama pasien (mis. panggilan hasil lab) → display menampilkan nama + suara "Bapak [nama], silakan menuju Pendaftaran." (pilihan "-" memakai sapaan netral "Kepada").
...
**Pemanggilan pasien poli + suara**: halaman `/admin` memiliki **menu bar** `[Pendaftaran | Poli]` — tab terakhir yang dipilih diingat otomatis. Di menu **Poli**, petugas melihat 3 kolom (Penyakit Dalam, Poli Anak, Poli Umum) berisi daftar nama pasien hari ini dengan tombol **Panggil** di samping tiap nama. Tombol berubah menjadi **Sudah Dipanggil** (abu-abu) setelah dipanggil; bisa diklik lagi untuk panggil ulang. Sistem menulis ke collection `poli_calls` (`name`, `ruangan`, `calledBy`, `calledAt`). Display Poli menerima event tersebut dan mengumumkan 2x: *"Kepada pasien atas nama [NAMA], silakan menuju ke [poli]."* (contoh: "poli Penyakit Dalam (dr. Andri)", "poli Anak") — panggilan kedua dibunyikan **setelah panggilan pertama selesai** + jeda 2,5 detik (tidak ada suara yang terpotong). Nama diucapkan dalam Title Case (Voice TTS mengeja nama ALL-CAPS huruf per huruf); `"dr."` dibacakan sebagai "dokter". Display Poli juga memerlukan **tap layar sekali** saat pertama dibuka (overlay suara, sama seperti display utama).
